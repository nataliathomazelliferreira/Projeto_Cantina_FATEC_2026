...
class Produto:
    def __init__(self, nome, preco_compra, preco_venda, data_vencimento, quantidade):
        ...
        self.__data_vencimento = datetime.strptime(data_vencimento, "%d/%m/%Y")
...
class ListaEstoque:
    ...
    def adicionar(self, produto):
        ...
        if self.__cabeca is None or produto.get_data_vencimento() < self.__cabeca.dado.get_data_vencimento():
            novo.proximo = self.__cabeca
            self.__cabeca = novo
        else:
            atual = self.__cabeca
            while atual.proximo and produto.get_data_vencimento() >= atual.proximo.dado.get_data_vencimento():
                atual = atual.proximo
...
class Venda:
    ...
    def exibir(self):
        print(f"\n  Cliente: {self.__cliente} | {self.__data.strftime('%d/%m/%Y %H:%M:%S')}")
        atual = self.__cabeca_itens
        while atual:
            print(f"    {atual.item}")
            atual = atual.proximo
...
def menu(estoque, registro):
    while True:
        print("\n===== MENU - CANTINA FATEC =====")
        print(" 1 - Ver estoque")
        print(" 2 - Gerar vendas automaticas")
        print(" 3 - Restaurar estoque")
